...
async function seedFixtures(): Promise<void> {
  ...
  await insertRows(
    ...
    [
      // 纯日额度模型（ADR-0003）：每池一条 UTC 日池级行；非空 user_id 为该用户的日限频计数行。
      ...["existing_auth", "new_registration", "base_business", "urgent_business"].map(
        (pool, i) => [
          `up_d_${i + 1}`,
          pool,
          "utc_day",
      ...
      ["up_u_1", "base_business", "utc_day", "2027-01-15", "u_001", 0, 1, 0, T0, T0 + DAY, T0, T0],
      [
        "up_u_2",
        "urgent_business",
        "utc_day",
