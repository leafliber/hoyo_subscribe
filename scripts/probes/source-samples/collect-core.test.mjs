...
const annSource = {
  ...
  list: {
    path: "/common/test_cn/announcement/api/getAnnList",
    params: {
      game: "test",
      game_biz: "test_cn",
      bundle_id: "test_cn",
  ...
  content: {
    path: "/common/test_cn/announcement/api/getAnnContent",
    params_key: "announcement_id",
  },
};
...
test("A-P0-SOURCE: 公告列表/正文 URL 构造（含 page/page_size 与 announcement_id）", () => {
  ...
  assert.equal(
    url,
    "https://ann-api.example.com/common/test_cn/announcement/api/getAnnList?game=test&game_biz=test_cn&bundle_id=test_cn&channel_id=1&lang=zh-cn&level=60&platform=pc&region=cn_test&uid=100000000&page=2&page_size=20",
  );
  const curl = buildAnnContentUrl(annSource, 12345);
  assert.ok(
    curl.endsWith(
      "getAnnContent?game=test&game_biz=test_cn&bundle_id=test_cn&channel_id=1&lang=zh-cn&level=60&platform=pc&region=cn_test&uid=100000000&announcement_id=12345",
    ),
  );
});
...
test("A-P0-SOURCE: 米游社列表/正文 URL 构造（type/last_id/page_size）", () => {
  ...
  assert.equal(
    url,
    "https://news.example.com/painter/wapi/getNewsList?gids=2&type=1&last_id=77&page_size=20",
  );
...
test("A-P0-SOURCE: parseAnnList 识别分组列表与扁平正文两种形状（synthetic 数据）", () => {
  ...
  assert.deepEqual(
    g.flatItems.map((i) => i.ann_id),
    [1, 2, 3],
  );
  assert.deepEqual(
    g.groups.map((x) => x.type_label),
    ["公告", "活动"],
  );
  ...
  const flat = JSON.stringify({
    retcode: 0,
    message: "OK",
    data: { list: [{ ann_id: 1, content: "<p>x</p>" }] },
  });
...
test("A-P0-SOURCE: parseNewsList 解析游标与 is_last（synthetic 数据）", () => {
  const body = JSON.stringify({
    ...
    message: "OK",
    data: {
      list: [{ post: { post_id: "1", uid: "0", post_status: { is_top: true } } }],
      last_id: 555,
      is_last: false,
    },
  });
  ...
  const last = parseNewsList(
    JSON.stringify({ retcode: 0, data: { list: [], last_id: 0, is_last: true } }),
  );
...
test("A-P0-SOURCE: 官方转义 t_gl 高亮标签与「（服务器时间）」注记不打断区间提取（synthetic 数据）", () => {
  const raw =
    '〓祈愿时间〓 &lt;t class="t_gl"&gt;2026/09/18 12:00&lt;/t&gt; ~ &lt;t class="t_gl"&gt;2026/10/06 17:59&lt;/t&gt;';
...
test("A-P0-SOURCE: isDateCarriedByImage 判定日期主要在图片承载", () => {
  const imgHeavy = isDateCarriedByImage(
    '<p><img src="a"><img src="b"><img src="c">祈愿开启！</p>',
    3,
  );
  assert.equal(imgHeavy.date_likely_in_image, true);
  const textHeavy = isDateCarriedByImage(
    "<p>祈愿时间：9月18日 12:00 ~ 10月6日 17:59，更多说明……</p>",
    1,
  );
